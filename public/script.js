...
// Vercel-dəki backend-in tam linki. Sayt öz domenində işlədiyi üçün
// boş saxlamaq kifayətdir (nisbi yol istifadə olunur) — amma sən
// backend-i ayrı domendə saxlayırsansa, buraya tam URL yaza bilərsən.
const API_BASE_URL = '';
...
async function loadHistory() {
  try {
    const res = await fetch(`${API_BASE_URL}/api/history?sessionId=${encodeURIComponent(getSessionId())}`);
    if (!res.ok) return;
